...
import NextBreadCrumbStatic from '@/components/common/next-breadcrumb-static'
...
// 這是SSG要產生靜態網頁的必要過程
export default function Post({ post, pathname }) {
  // const { asPath } = useRouter()
  const { title, content } = post

  return (
    <>
      <NextBreadCrumbStatic pathname={pathname} />
      <h1>Title: {title}</h1>
      <p>Path: {pathname}</p>
      <p>Content: {content}</p>
    </>
